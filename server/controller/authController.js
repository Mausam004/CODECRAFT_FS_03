...
export const verifyOtp = async (req, res) => {
 ...
 const otp_expires = new Date(Date.now());
 console.log("current ",otp_expires)

 const sql = "SELECT * FROM register WHERE email = ? and otp = ? and otp_expires > ? ";

 db.query(sql, [email, enteredOTP, otp_expires], (err, result) => {
...
export const sendResetPasswordMail = async (req, res) => {
  ...
  const otp_expires = new Date(Date.now() +  60 * 1000);
  const email = req.body.email;

  const sql = "UPDATE register SET otp = ? , otp_expires = ? WHERE email = ?";

  db.query(sql, [otp, otp_expires, email], (err, result) => {
    if (err) {
...
export const resendResetPasswordMail = async (req, res) => {
  ...
  const sqlSelect = "SELECT otp_expires FROM register WHERE email = ?";
  ...
  db.query(sqlSelect, [email], (err, results) => {
    ...
    const sqlUpdate = "UPDATE register SET otp = ?, otp_expires = ? WHERE email = ?";
